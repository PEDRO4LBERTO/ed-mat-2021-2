...
for(let est of estadosNe){
   densidade(`${est.area}, ${est.populacao}`)
   delete est.sigla
}
...
const objeto = obj => {
   for(let estad in estadosNe){
      console.log(`${estad} -> ${estadosNe[estad]}`)
   }
   return 
}
...
estadoss.unshift('Alagoas')
estadoss.push('Sergipe')
estadoss.splice(1,0,'Bahia')
estadoss.splice(2,0,'Ceará')
estadoss.splice(3,0,'Maranhão')
estadoss.splice(4,0,'Paraíba')
estadoss.splice(5,0,'Pernambuco')
estadoss.splice(6,0,'Piauí')
estadoss.splice(7,0,'Rio Grande do Norte')
